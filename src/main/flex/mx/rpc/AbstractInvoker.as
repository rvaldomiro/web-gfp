...
package mx.rpc
{
	...
	import common.components.serviceFeedback.ServiceFeedback;
...
//			{
//				trace(FlexSprite.mx_internal::VERSION);
//				
//				setTimeout(function():void
//				{
//					throw new Error("The SDK version was updated. In order to use the ServiceFeedback Class you have to generate a new Version using the latest AbstractInvoker");
//				}, 1);
//			}
		...
		{
			var msgEvent:MessageEvent = MessageEvent.createEvent(MessageEvent.MESSAGE
																 , event.message);
			...
			if ((token == null) && (AsyncMessage(event.message).correlationId != null)
				&& (AsyncMessage(event.message).correlationId != "") && (event.faultCode
			...
			{
				var fault:Fault = new Fault(event.faultCode, event.faultString, event
											.faultDetail);
				fault.content = event.message.body;
				fault.rootCause = event.rootCause;
				var faultEvent:FaultEvent = FaultEvent.createEvent(fault, token, event
																   .message);
		...
		mx_internal function invoke(message:IMessage, token:AsyncToken = null):AsyncToken
		{
			...
			catch (e:MessagingError)
			{
				...
				var errorText:String = resourceManager.getString("rpc", "cannotConnectToDestination"
																 , [asyncRequest
																	.destination]);
			...
			ServiceFeedback.addResponder(token);
		...
		mx_internal function monitorRpcEvent(event:AbstractEvent):void
		{
			if (NetworkMonitor.isMonitoring())
			{
				...
				{
					NetworkMonitor.monitorResult(event.message, mx.rpc.events.ResultEvent(event)
												 .result);
				}
				else if (event is mx.rpc.events.FaultEvent)
				{
					//trace(" AbstractInvoker: MonitorFault - message:" + event.message);
					NetworkMonitor.monitorFault(event.message, mx.rpc.events.FaultEvent(event)
												.fault);
				}
			}
		}
		...
		mx_internal function processResult(message:IMessage, token:AsyncToken):Boolean
		{
			...
			if (makeObjectsBindable && (body != null) && (getQualifiedClassName(body)
				== "Object"))
			{
		...
		mx_internal function resultHandler(event:MessageEvent):void
		{
			...
			{
				dispatchEvent(new flash.events.Event(BINDING_RESULT));
				var resultEvent:ResultEvent = ResultEvent.createEvent(_result, token
																	  , event.message);
